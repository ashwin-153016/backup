...
public class App 
{
    ...
    public void createAccount(){
    	...
    	try {
		   ...
		   if(ok == true) {
			   HashMap<String,String> cust_acc = ws.createAccount(c,a);
			  if(cust_acc!=null) {
				  System.out.println("Customer ID: "+ cust_acc.keySet().toString()+"\nAccount number: "+cust_acc.values().toString());
				  
				  String transId = ws.dwTransaction("deposit",LocalDate.now(), cust_acc.keySet().toString(),"","success",a.getAccount_balance());
    ...
    public void deposit() {
    	...
    	try {
			...
			if(totalAmt == null) {
				...
				String cust_id = ws.getCustomerId(acc);
				String transId = ws.dwTransaction("deposit",LocalDate.now(), cust_id,"","success",amt);
				System.out.println("Transaction Id: "+transId);
			}
		} catch (WalletException e) {
			System.out.println();
    ...
    public void withdraw() {
    	...
    	try {
			...
			if(totalAmt == null) {
				...
				String cust_id = ws.getCustomerId(acc);
				String transId = ws.dwTransaction("withdrawal",LocalDate.now(),"",cust_id,"success",amt);
				System.out.println("Transaction Id: "+transId);
    ...
    public void fundTransfer() {
    	...
    	try {
			 String balance = ws.fundTransfer(acc, amt, rAcc);
				 ...
				 String cust_id = ws.getCustomerId(acc);
				 String cust_id1 = ws.getCustomerId(rAcc);
				 String transId = ws.transferTransaction("transafer",LocalDate.now(), cust_id,cust_id1,"success",amt);
				 System.out.println("Transaction ID: "+transId);
			  ...
			  if(check == true) {
				  ...
				  if(tMap.size() == 0) {
				    ...
				    for(Map.Entry<String, Transaction> entry: tMap.entrySet()) {
					 ...
					 else if(entry.getValue().getTransaction_type().toString().equals("deposit")){
					  System.out.println("Account number         : "+ entry.getValue().getTransaction_sender().toString());
					 }
					 else if(entry.getValue().getTransaction_type().toString().equals("withdrawal")){
						  System.out.println("Account number         : "+ entry.getValue().getTransaction_receiver().toString());
						 }
					 System.out.println("======================================");
					 }
				  }
